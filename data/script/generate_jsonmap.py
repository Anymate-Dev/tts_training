    ...
    for lang in ['en', 'zh']:
        ...
        for person in os.listdir(lang_path):
            ...
            if os.path.isdir(person_path):
                ...
                files_info = {}
                file_names = [os.path.splitext(f)[0] for f in os.listdir(person_path)]
                unique_file_names = set(file_names)
                for base_name in unique_file_names:
                    wav_path = os.path.join(person_path, base_name + '.wav')
                    lab_path = os.path.join(person_path, base_name + '.lab')
                    if os.path.isfile(wav_path) and os.path.isfile(lab_path):
                        ...
                        catalog[lang][person] = list(files_info.values())[:5]  # Only include up to 5 pairs
    return catalog
